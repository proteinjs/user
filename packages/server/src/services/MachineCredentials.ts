 ...
 *
 * Generates a strong random password (never human-chosen), stores its sha256 on the account row
 * (the plaintext is never persisted or logged), kills the account's existing sessions (the old
...
export class MachineCredentials implements MachineCredentialsService {
  ...
  async mintCredential(email: string): Promise<MintedMachineCredential> {
    ...
    await db.update(tables.User, { id: user.id, password: sha256(password).toString() });
